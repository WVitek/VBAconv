...
Public Type PARAMCALC
 ...
 FlowDirection As FLOW_DIRECTION         '   показывает, как поток направлен относительно координат по длине
 'FlowAtStart As FLOW_DIRECTION_AT_START  '   показывает, что точка начального давления соответвует потоку втекающему в трубу (забой)
 'TempDirection As TEMP_DIRECTION         '   если  false  то значит в начальной точке поток вытекает из трубы (устье)
 tempMethod As TEMP_CALC_METHOD          ' метод расчета температуры в трубе
End Type
...
Public Type PIPE_FLOW_PARAMS
  ...
  Muo_cP As Double          ' вязкость нефть в потоке
  Muw_cP As Double          ' вязкость воды в потоке
  Mug_cP As Double          ' вязкость газа в потоке
  MuMix_cP As Double        ' вязкость смеси в потоке
  ...
  Qo_m3day As Double        ' расход нефти в рабочих условиях
